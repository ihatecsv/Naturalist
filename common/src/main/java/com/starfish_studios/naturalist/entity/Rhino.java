...
public class Rhino extends Animal implements GeoEntity {
    ...
    private <E extends Rhino> PlayState predicate(final AnimationState<E> event) {
        if (this.stunnedTick > 0) {
            event.getController().setAnimation(RawAnimation.begin().thenLoop("rhino.stunned"));
            event.getController().setAnimationSpeed(1.0F);
        } else if (event.isMoving()) {
            if (this.isSprinting()) {
                event.getController().setAnimation(RawAnimation.begin().thenLoop("rhino.run"));
                event.getController().setAnimationSpeed(3.0F);
            } else {
                event.getController().setAnimation(RawAnimation.begin().thenLoop("rhino.walk"));
                event.getController().setAnimationSpeed(1.0F);
            }
        } else if (this.hasChargeCooldown() && this.hasTarget()) {
            event.getController().setAnimation(RawAnimation.begin().thenLoop("rhino.foot"));
            event.getController().setAnimationSpeed(1.0F);
        } else {
            event.getController().setAnimation(RawAnimation.begin().thenLoop("rhino.idle"));
            event.getController().setAnimationSpeed(1.0F);
        }
    ...
    private <E extends Rhino> PlayState attackPredicate(final AnimationState<E> event) {
        if (this.swinging && event.getController().getAnimationState().equals(AnimationController.State.STOPPED)) {
            ...
            event.getController().setAnimation(RawAnimation.begin().thenPlay("rhino.attack"));
            this.swinging = false;
        }
        return PlayState.CONTINUE;
    }
